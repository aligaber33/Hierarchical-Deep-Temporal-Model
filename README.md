...
│
├── *.pth                      # Saved model checkpoints (Git LFS pointers — see below)
...
| Script | Model | Input | Temporal modeling | Notes |
|---|---|---|---|---|
...
| `Full_Model.py` | `TwoStageModelB7` | Per-player crop sequence + presence mask | **Two-stage**: per-player LSTM over time → max-pool over players per frame → second LSTM over the pooled group sequence | The full hierarchical model described in the paper: person-temporal stage + group-temporal stage. |
...
|---|---|---|
| `Pytorch_baseline_b1.pth` | `Baseline_B1.Image_Classifier` | ~94 MB |
| `Pytorch_baseline_b2.pth` | `Baseline_B2.PersonClassifier` | ~1 MB |
| `Pytorch_baseline_b3.pth` | `Baseline_B3.GroupActivityRecognition` | ~13 MB |
| `Pytorch_baseline_b4.pth` | `Baseline_b4.TemporalClassifier` | ~95 MB |
| `Pytorch_baseline_b5.pth` | `Baseline_b5.TemporalModelB5` | ~97 MB |
| `Pytorch_baseline_b6.pth` | `Baseline_B6.TwoStageModelB6` (raw checkpoint dict — see below) | ~97 MB |
| `Final_model_weights.pth` | `Full_Model.TwoStageModelB7` | ~118 MB |
...
`Annotation_loader.py` builds an in-memory (and optionally pickled) index of the dataset, and can visualize a clip's tracked bounding boxes as a debug video:
...
Each `Baseline_*.py` / `Full_Model.py` file is self-contained and runnable as a script — it builds its dataset, trains for a fixed number of epochs, evaluates on a held-out split, and saves a checkpoint:
...
model, epoch = load_checkpoint(model, "Final_model_weights.pth", device=device)
model.eval()
```
...
- **Hard-coded paths differ between scripts.** `Baseline_B1.py`–`Baseline_b5.py` use relative paths (`Path("videos-splitted")`, `Path("features/...")`), while `Baseline_B6.py` and `Full_Model.py` use absolute Lightning AI Studio paths (`/teamspace/studios/this_studio/videos-splitted`). If you're not running inside that same Lightning Studio environment, edit `vids_root` / `annot_root` at the top of those two files to match your local dataset location.
